...
const tabs = [
  //{ id: "sms", label: "SMS", module: "tabs/sms.js" },
...
// Pages that do not appear in the navigation bar can be declared here.
// The key corresponds to the page identifier (e.g. 'messages') and the
// value is the path to the JavaScript module implementing the page.
...
function loadTab(tabId, anchorId = null) {
  // Determine whether this is a defined navigation tab or an additional page.
  ...
  // Highlight only if this is a visible navigation tab.
  ...
  // Insert the script for the page.
  ...
  // Store last tab only if this is a navigation tab.
  if (isNavigationTab) {
    localStorage.setItem('lastTab', tabId);
  }

  // Update URL hash.
  const expectedHash = anchorId && isNavigationTab
    ? `#${tabId}:${anchorId}`
    : `#${tabId}`;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // If there is a hash present use that in preference to the saved tab.
  const initialTab = hashTab || savedTab || 'activity';
...
window.addEventListener('hashchange', () => {
  const [hashTab, anchorId] = window.location.hash.slice(1).split(':');
  if (!hashTab) return;
  const isNavTab = tabs.some(t => t.id === hashTab);
  const isExtraPage = Object.prototype.hasOwnProperty.call(additionalPages, hashTab);
  if (isNavTab || isExtraPage) {
