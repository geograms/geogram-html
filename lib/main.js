...
const tabs = [
  { id: "activity", label: "Activity", icon: "fa-solid fa-list", module: "tabs/activity.js" },
  { id: "messages", label: "Messages", icon: "fa-solid fa-envelope", module: "tabs/messages.js" },
  //{ id: "map", label: "Map", icon: "fa-solid fa-globe", module: "tabs/map.js" },
  //{ id: "docs", label: "Docs", icon: "fa-solid fa-folder", module: "tabs/docs.js" },
  { id: "config", label: "Config", icon: "fa-solid fa-gear", module: "tabs/config.js" },
  //{ id: "profile", label: "Profile", icon: "fa-solid fa-user", module: "tabs/profile.js" }
];
...
function generateTabs() {
  ...
  desktopTabs.innerHTML = tabs.map(
    tab => `<button class="tab" data-id="${tab.id}" title="${tab.id}">
              <i class="${tab.icon}" aria-hidden="true"></i>
              <span class="sr-only">${tab.id}</span>
            </button>`
  ).join("");
}
...
function loadTab(tabId, anchorId = null) {
  ...
  // Call cleanup function for the current tab before removing
  const currentTab = localStorage.getItem("currentTab");
  if (currentTab) {
    // Call cleanup functions if they exist
    if (currentTab === "messages" && typeof window.cleanupMessages === "function") {
      window.cleanupMessages();
    } else if (currentTab === "activity" && typeof window.cleanupActivity === "function") {
      window.cleanupActivity();
    }
  }

  // Store the new current tab
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Handle both : and @ as separators (: for anchors, @ for groups)
  const hashPart = window.location.hash.slice(1);
  const [hashTab, anchorId] = hashPart.includes(':') ? hashPart.split(":") :
                                hashPart.includes('@') ? [hashPart.split('@')[0], null] :
                                [hashPart, null];
