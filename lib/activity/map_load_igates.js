...
(function() {
    ...
    function loadFromCache(store, alsoFetchFresh = false) {
        ...
        request.onsuccess = function() {
            ...
            if (cachedData && cachedData.length > 0) {
                ...
                // Check if cache seems incomplete (less than expected)
        ...
        function processBatch() {
            ...
            try {
                ...
                if (processedCount < data.length) {
                    ...
                    setTimeout(processBatch, 5); // Small delay to prevent blocking
                } else {
                ...
                // Try to continue with next batch after a longer delay
                processedCount = batchEnd;
                if (processedCount < data.length) {
     ...
     * Properly reset state to allow reloading on return
     */
    window.cleanupIGateStations = function() {
        if (clusterGroup && currentMap) {
            currentMap.removeLayer(clusterGroup);
            clusterGroup.clearLayers(); // Clear all markers from cluster
            clusterGroup = null;
        }
        currentMap = null;
        isLoading = false;
        processedCount = 0;
        // Reset loadedOnce to allow fresh loading when returning to this view
        // This ensures stations reload even if there were errors during navigation
        loadedOnce = false;
        console.log('IGATE stations cleanup completed');
    };

    /**
     * Clear all cached IGATE data (useful for debugging cache issues)
     */
