// --- nearby.js (updated, with GeoChat integration) ---

// === GEOCHAT GLUE: identity from local browser cache (additive) ===
...
async function activityStartLocalMaps() {
  ...
  // Chat logic for Nearby (fallback/local echo)
  ...
  // Event listeners for chat (fallback/local echo)
  setTimeout(() => {
    const sendBtn = document.getElementById('sendChatBtn');
    if (sendBtn) sendBtn.onclick = sendNearbyMessage;
    const emojiBtn = document.getElementById('emojiBtn');
  ...
  // === GEOCHAT GLUE: minimal wiring (additive; keeps your existing UI) ===
  (function wireGeoChat() {
    const messagesEl = document.getElementById("chatMessages");
    const inputEl    = document.getElementById("chatInput");
    const sendBtnEl  = document.getElementById("sendChatBtn");

    // Live getters so GeoChat uses current map center & radius
    const getLocation = () => ({ lat: center.lat, lon: center.lng });
    const getRadius   = () => radius;

    // Credentials from local cache
    const { npub, nsec, callsign } = getChatIdentityFromCache();

    // Start GeoChat only if everything needed is present; otherwise leave your original chat intact
    if (window.GeoChat && npub && nsec && callsign && messagesEl && inputEl && sendBtnEl) {
      // Avoid double-binding with fallback handlers:
      try { sendBtnEl.onclick = null; } catch {}
      try { inputEl.onkeydown = null; } catch {}

      window.GeoChat.init({
        npub, nsec, callsign,
        getLocation, getRadius,
        messagesEl, inputEl, sendBtnEl,
        onRender: ({ count }) => { /* optional: update counters, badges, etc. */ }
      });

      // Optional: change polling interval if desired
      // window.GeoChat.configure({ pollMs: 60_000 });
    } else {
      // GeoChat not available or creds missing -> keep your original behavior (fallback)
      if (!npub || !nsec || !callsign) {
        console.warn("GeoChat credentials not found in localStorage (npub/nsec/callsign). Using fallback chat.");
      } else if (!window.GeoChat) {
        console.warn("GeoChat not loaded (window.GeoChat missing). Using fallback chat.");
      }
    }
  })();
  ...
  window.cleanupNearbyMap = function() {
    ...
    // Stop GeoChat poller if active
    if (window.GeoChat && typeof window.GeoChat.destroy === "function") {
      window.GeoChat.destroy();
    }
