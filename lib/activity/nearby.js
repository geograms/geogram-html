...
//  - Status now reflects the LAST SUCCESSFUL server fetch (serverAt)
//  - Human readable "Last updated": seconds (<1m) → minutes (<60m) → hours (<24h) → days (>=24h) with " ago"
//
// Public API:
...
(function () {
  ...
  // For table: seconds (<1m), minutes (<60m), hours (<24h), days (>=24h) — with " ago"
  function fmtLastUpdated(ts) {
    ...
    if (diff < minute) {
      const secs = Math.max(0, Math.round(diff / second));
      return `${secs} s ago`;
    }
    if (diff < hour) {
    ...
    if (diff < day) {
      const hrs = Math.max(1, Math.round(diff / hour));
      return `${hrs} h ago`;
    }
  ...
  function setCached(key, data) {
    const now = Date.now();
    const obj = { data, fetchedAt: now, serverAt: now }; // serverAt = last successful fetch
  ...
  function fetchLive(key, lat, lon, radiusKm) {
    ...
    const p = jsonpFetch(lat, lon, radiusKm)
      ...
      .then(data => {
        setCached(key, data); // updates serverAt to "now"
        return data;
      })
  ...
  async function mountMapCard(parentEl, opts) {
    ...
    ensureLeafletLoaded().then(() => {
      ...
      // Render cached data immediately when map is ready
      if (pendingDevices) {
    ...
    // 1) If cached exists, show it immediately and mark with last serverAt
    ...
    if (cached?.data && !hasRenderedInitialData) {
      ...
      updateTable(cached.data); // Show cached data in table immediately
      pendingDevices = cached.data; // Store for when map loads
    ...
    function refreshLive() {
      fetchLive(cacheKey, lat, lon, radiusKm)
        .then((data) => {
          updateTable(data);
          updateMap(data);
          // Last successful fetch is "now"
          statusEl.textContent = `Updated: ${fmtClock(new Date())}`;
        })
        .catch((e) => {
          // Keep previous "Updated:" timestamp visible; only show error if nothing cached
          if (!cached?.data) {
