...
function sendToRadio() {
  ...
  const progressDisplay = document.getElementById('sendProgress');

  // If already sending, cancel
  if (sending) {
  ...
  let span = document.getElementById('sendProgress');
  if (!span) {
    span = document.createElement('span');
    span.id = 'sendProgress';
  ...
  progressInterval = setInterval(() => {
    progressValue += 10;
    document.getElementById('sendProgress').textContent = `(${progressValue}% done)`;

    if (progressValue >= 100) {
      clearInterval(progressInterval);
      button.textContent = 'Send to radio';
      document.getElementById('sendProgress').textContent = '';
      sending = false;
      progressValue = 0;
    }
  }, 1000);
