...
(function () {
  ...
  function injectResponsiveStyles() {
    ...
    style.textContent = `
      ...
      .nm-map { height: clamp(300px, 50vh, 520px); width: 100%; position: relative; }
      ...
      /* Muted base (keeps rivers/parks faintly colored instead of full grayscale)
         Tweak --tile-filter if you want more/less color pop. */
      .nm-map { --tile-filter: grayscale(1.5) saturate(1.0) contrast(1.0) brightness(.97); }
      .nm-map .leaflet-tile {
  ...
  async function mountMapCard(parentEl, opts) {
    ...
    function updateMap(devices, icons) {
      ...
      // UPDATED: Circle with no fill (only stroke)
      L.circle([lat, lon], {
        radius: radiusKm * 1000,
        color: COLOR_RADIUS_RING,
        weight: 1.2,
