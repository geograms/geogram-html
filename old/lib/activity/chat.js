...
if (!window.__GeogramChatInitialized) {
  ...
  (function () {
    ...
    function getChatIdentityFromCache() {
      ...
      if (npub === null || nsec === null || callsign === null) {
        if (debug) console.debug("[chat] Missing identity in localStorage (privkey, pubkey, username)");
        generateNewNostrAndCallsign();
        return getChatIdentityFromCache(); // re-fetch after generation
      }
    ...
    function appendMessage({ text, timestamp, outgoing, pendingId = null, authorName }) {
      ...
      wrapper.innerHTML = `
    ${!outgoing && authorName
          ? `<div style="font-size:0.72em;font-weight:600;opacity:.8;margin:0 0 4px 4px;align-self:${alignment};">
  ...
  `;
    ...
    async function readMessages() {
      ...
      for (const d of unique) {
        ...
        const isOutgoing =
          (d?.author || "").toUpperCase() === (callsign || "").toUpperCase();

        appendMessage({
          text: d?.content ?? "",
          timestamp: formatTs(d?.timestamp),
          outgoing: isOutgoing,
          // show others' name only (WhatsApp-style)
          authorName: isOutgoing ? undefined : (d?.author ?? "").trim()
        });
      }
    ...
    function startPolling() {
      ...
      __pollTimer = setInterval(() => {
        // Don't spam when the tab isn’t visible
    ...
    window.addEventListener("visibilitychange", () => {
      if (!document.hidden) {
        // Fetch immediately on return to the tab
        readMessages().catch(console.warn);
      }
    });
