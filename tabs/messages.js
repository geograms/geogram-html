...
var avatarInitials = { group1: 'FG', group2: 'WP', user123: 'JD', user456: 'AS' };
...
function _requireDeps() {
  ...
  if (typeof getChatIdentityFromCache !== 'function') throw new Error('getChatIdentityFromCache() missing');
}
...
function render() {
  ...
  // Hook up actions
  document.getElementById('btn-reload').addEventListener('click', _loadConversations);
  document.getElementById('searchInput').addEventListener('input', _searchFilter);
...
function loadMessages() { // keep your existing alias
  if (typeof window.loadTab === 'function') {
    window.loadTab('messages');
    return;
  }
  ...
  script.id = 'dynamic-tab';
  script.onload = () => { if (typeof render === 'function') render(); };
  document.body.appendChild(script);
  window.location.hash = '#messages';
}

function cleanupMessages() { /* no-op */ }
